...
| Pílula de estado da ocorrência | o estado **não aparecia** na página da ocorrência | `.ocorrencia-estado__pilula`, a mesma `c-badge` do cartão e da home, alimentada por `estado_visual` + `prettify_state` | **CREATE** | `_ocorrencia.scss`, `report/_ocorrencia-cabecalho.html` |
| Abas Detalhes | Atualizações | a página era uma coluna só | `.ocorrencia-abas`, duas visões no mesmo documento; `role="tab"` posto por script, porque sem ele nada é escondido | **CREATE** | `_ocorrencia.scss`, `catanduva-map.js` |
| Linha do tempo da ocorrência | lista de atualizações do upstream, no fim da página | `.ocorrencia-linha`, um `ol` com ponto e fio; o registro é o primeiro evento, e a origem de cada atualização vem de `atualizacao_da_equipe` | REPLACE | `_ocorrencia.scss`, `report/_ocorrencia-atualizacoes.html` |
| Galeria de fotos da ocorrência | miniaturas soltas do upstream, sem limite declarado | `.ocorrencia-fotos`, grade de até 3 com "(N de 3)" e o limite escrito | REPLACE | `_ocorrencia.scss`, `report/_ocorrencia-detalhes.html` |
| Visualizador de fotos | o `rel="fancy"` do upstream | `.ocorrencia-visor`, camada própria com contador, setas que somem nas pontas e miniaturas; devolve o foco ao sair | **CREATE** | `_ocorrencia.scss`, `catanduva-map.js` |
